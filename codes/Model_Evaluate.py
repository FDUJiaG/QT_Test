...
def model_eva(stock, state_dt, para_window, para_dc_window):
    ...
    sql_final_insert = "insert into model_ev_resu(state_dt,stock_code,para_window,para_dc_window,acc,recall,f1,acc_neg,bz,predict)values('%s','%s','%d','%d','%.4f','%.4f','%.4f','%.4f','%s','%s')" % (state_dt, stock, para_window, para_dc_window, acc, recall, f1, acc_neg, 'svm', str(predict))
...
if __name__ == '__main__':
    ...
    # stock_pool = ['002063.SZ']
    for stock in stock_pool:
        ans = model_eva(stock, '2018-03-01', 99, 1000)
    print('All Finished !!')